...
export const AIChatSidePanel: React.FC<AIChatSidePanelProps> = ({
  ...
  const buildCodeContext = () => {
    const isCodeActionMode =
      chatMode === "review" || chatMode === "fix" || chatMode === "optimize";
    const maxCodeLength = isCodeActionMode ? 8000 : 2000;
    ...
    const context: {
      ...
      codeContent?: string;
      cursorPosition?: { line: number; column: number };
      attachments?: Array<{ name: string; language: string; content: string }>;
    } = {};

    if (activeFileName) context.fileName = activeFileName;
    if (activeFileLanguage) context.language = activeFileLanguage;
    if (activeFileContent) {
      context.codeContent = activeFileContent.substring(0, maxCodeLength);
    }
    if (cursorPosition) context.cursorPosition = cursorPosition;

    if (attachments.length > 0) {
      context.attachments = attachments.map((file) => ({
        name: file.name,
        language: file.language,
        content: file.content.substring(0, isCodeActionMode ? 4000 : 1000),
      }));
    }
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    const hasCodeContext =
      ...
      attachments.some((file) => file.content.trim());

    if (isLoading) return;
    if (!input.trim() && !(isCodeActionMode && hasCodeContext)) return;
    ...
    const newMessage: ChatMessage = {
      ...
      content:
        input.trim() ||
        (chatMode === "review"
          ? `Review ${activeFileName || "the attached code"}`
          : chatMode === "fix"
          ? `Fix issues in ${activeFileName || "the attached code"}`
          : `Optimize ${activeFileName || "the attached code"}`),
      timestamp: new Date(),
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (streamResponse && contentType.includes("text/plain")) {
          setMessages((prev) => [
            ...prev,
            {
              role: "assistant",
              ...
              {filteredMessages.length === 0 && !isLoading && (
                <div className="text-center text-zinc-500 py-16">
                  ...
                  <div className="grid grid-cols-2 gap-2 max-w-lg mx-auto">
                    {[
                      {
                        text: "Review my React component for performance",
                        mode: "review" as const,
                      },
                      {
                        text: "Fix TypeScript compilation errors",
                        mode: "fix" as const,
                      },
                      {
                      ...
                      {
                        text: "Refactor code for better maintainability",
                        mode: "review" as const,
                      },
                    ].map((suggestion) => (
                      <button
                ...
                <Textarea
                  placeholder={
                    chatMode === "chat"
                      ...
                      : chatMode === "review"
                      ? activeFileName
                        ? `Review ${activeFileName} (or add specific focus areas)...`
                        : "Describe what you'd like me to review, or attach code..."
                      : chatMode === "fix"
                      ? activeFileName
                        ? `Describe the issue in ${activeFileName}, or send to auto-fix...`
                        : "Describe the issue or attach code to fix..."
                      : activeFileName
                      ? `Optimize ${activeFileName} (or add specific goals)...`
